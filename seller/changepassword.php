...
if(isset($_POST['submit']))
{
    ...
    include '../connect.php';
    if (count($_POST) > 0) {
        $result = mysqli_query($conn, "SELECT *from seller WHERE email='" . $_SESSION['email'] . "'");
        $row = mysqli_fetch_array($result , MYSQLI_ASSOC);
        if (($_POST["currentPassword"] == $row["password"]) && $_POST["newPassword"]==$_POST["confirmPassword"]){
            mysqli_query($conn, "UPDATE seller set password='" . $_POST["newPassword"] . "' WHERE email='" . $_SESSION["email"] . "'");
