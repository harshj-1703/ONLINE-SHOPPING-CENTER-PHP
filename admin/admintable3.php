<?php
 ...
 {
    $id = $_GET["delete"];
    $upd = "UPDATE seller SET status = '0' WHERE email = '$id'";
    $conn->query($upd);
 }
 if(isset($_POST["home"]))
 {
...
$result1 = mysqli_query($conn,"SELECT * FROM seller where status='1'");
$limit = 4;
...
$sql = "SELECT fname,lname,email,mo1,birthdate,gender,address,city,state,country FROM seller where status='1' LIMIT $initial_page,$limit";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
      ...
      <td><button id='deletebutton' onclick=location.href='./admintable3.php?delete=".$row["email"]."'>Delete</button></td></tr>";
    }
    echo "</table>";
  } else {
    echo "0 results";
...
for($page_number = 1; $page_number<= $total_pages; $page_number++) {
  echo '<button id="linkbutton"><a href = "./admintable3.php?page=' . $page_number . '" id="pageno">' . $page_number . ' </a></button>&nbsp;&nbsp;';  
}?>
</td></tr></table><br><br><?php
$conn->close();
?>
    <table><tr>
    <form method="post" action="./admintable3.php">
        <td>
