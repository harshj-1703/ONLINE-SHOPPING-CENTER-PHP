...
if(isset($_POST['submit']))
    { 
        ...
        $target_dir = "../imges/sellerprofile/";
        ...
        $check_email = mysqli_query($conn, "SELECT email FROM seller where email = '$email' and status='1'");
        $check_emailold = mysqli_query($conn, "SELECT email FROM seller where email = '$email' and status='0'");
        $check_mo1 = mysqli_query($conn, "SELECT mo1 FROM seller where mo1 = '$mo1' and status='1'");
        $check_mo1old = mysqli_query($conn, "SELECT mo1 FROM seller where mo1 = '$mo1' and status='0'");
        if(mysqli_num_rows($check_emailold) > 0){
            $sql = "UPDATE seller SET fname='$fname',lname='$lname',mname='$mname',birthdate='$birthdate',
            ...
            window.location.replace("./sellerregistration.php");</script>';
        }
        ...
        {
            $sql = "INSERT INTO seller(`fname`,`mname`,`lname`,`birthdate`,`gender`,`mo1`,
            ...
            try {
                ...
                $mail->Body = 'HI '.$fname.' '.$lname. ' YOUR REGISTRATION ON SELLING PRODUCT, YOU <b>' .$email. '</b> 
                SUCCESSFULLY DONE NOW YOU CAN LOGIN. THANK YOU FOR YOUR REGISTRATION.';
                $mail->AltBody = '';
...
<head>
    ...
    <title>ONLINE SELLING CENTER</title>
...
<body >
    <table border="0"  cellspacing="10" cellpadding="10">
        <tr>
            <th>
                    <u><h1 id="head">ONLINE SELLING REGISTRATION</h1></u>
            </th>
            <td>
    ...
    <form name="registrationform" method="post" action="./sellerregistration.php" enctype="multipart/form-data">
        <table border="0"  cellspacing="10" cellpadding="10">
            <tr>
                <td>
